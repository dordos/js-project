(() => {
  ...
  function initScale(element) {
    ...
    function getZoomRate({ containerSize, target }) {
      ...
      return target.width * verticalValue > containerWidth
        ? horizontalValue
        : verticalValue;
    }
  ...
  function createPath(
    isAnswer,
    { svgContainer, stroke, answerStroke, strokeWidth }
  ) {
  ...
  class DrawLine {
    constructor({ container, draggableDropElement = true, sound, path = {} }) {
      this.binding();
    ...
    endDrag(__obj, DRAG, event) {
      ...
      // get array [drag, drop]
      const answerGroup =
        this.droppedObj && getAnswerGroup(__obj.value, this.droppedObj.value);

      //정답시 사운드 재생
      if (__obj.DOM.dataset.idx == this.droppedObj.DOM.dataset.dropAnswer) {
        this.sound.forEach((item) => {
          item.pause();
          item.currentTime = 0;
        });

        this.sound[__obj.DOM.dataset.idx].play();
      }

      // 정오답 비교 없이 진행
      const isCorrect =
        this.droppedObj && // drop 존재 유무
    ...
    insertUserValue() {
      this.userValue.value = this.answeredGroup
        .map((answer) => `${answer[0].index},${answer[1].index}`)
        .join(' ');
    }
    ...
    drawAnswer() {
      ...
      window.callback.showAnswer && window.callback.showAnswer(this);
    }
    //정답일경우
    correct(__drag, __drop) {
      ...
      this.newLine = {};
console.log(window.callback.correct)
      window.callback.correct && window.callback.correct(__drag, __drop);
    }

    incorrect(__obj) {
      this.removeNewLine();

      window.callback.incorrect && window.callback.incorrect(__drag, __drop);
    }
    ...
    reset() {
      ...
      window.callback.reset && window.callback.reset(this);
    }
  }
  ...
  function runDrawLine() {
    const jong = new Audio('./media/mp3/page105/jong_sound1.mp3');
    const magpie = new Audio('./media/mp3/page105/magpie_sound1.mp3');
    const piano = new Audio('./media/mp3/page105/piano_sound1.mp3');
    const abc = new Audio('./media/mp3/page105/drum_sound1.mp3');
    const firetruck = new Audio('./media/mp3/page105/firetruck_sound1.mp3');

    const answerSound = [jong, magpie, piano, abc, firetruck];
