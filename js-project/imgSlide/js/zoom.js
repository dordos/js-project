...
$(function(){
  ...
  $('.overBox1').css("background-size", parseInt(target.css('width')) * zoom + 'px ' + parseInt(target.css('height')) * zoom + 'px');
  ...
  function magnifys(x, y) { 
      ...
      if( saveZoom != zoom ){
          ...
          $('.overBox1').css("background-size", TargetWidth * zoom + 'px ' + TargetHeight * zoom + 'px');
          
          if( ItemX == TargetWidth && ItemY == TargetHeight ){
              var bgPosX = (-TargetWidth / 2 * (zoom - 1));
              var bgPosY = (-TargetHeight / 2 * (zoom - 1));
              $('.overBox1').css("background-position", bgPosX + 'px ' + bgPosY + 'px');

              var Xper = (-bgPosX / (TargetWidth * (zoom-1))) * 100;
              var Yper = (-bgPosY / (TargetHeight * (zoom-1))) * 100;
              $('.overBox1').css('transform-origin', Xper + '% ' + Yper + '%');
          } else {
              var bgPosX = (-TargetWidth / (-(ItemX - TargetWidth) / posX) * (zoom - 1));
              var bgPosY = (-TargetHeight / (-(ItemY - TargetHeight) / posY) * (zoom - 1));
              $('.overBox1').css("background-position", bgPosX + 'px ' + bgPosY + 'px');
  ...
  $(".overBox1").on(downEvent, function(e) {
      ...
      oriX = (($('.overBox1').width() - ($('.overBox1').width() / zoom)) / 100) * ((Number(ori[0].replace('px' ,'')) / $('.overBox1').width()) * 100);
      oriY = (($('.overBox1').height() - ($('.overBox1').height() / zoom)) / 100) * ((Number(ori[1].replace('px' ,'')) / $('.overBox1').height()) * 100);
      ...
      $(document).on(moveEvent, function(e) {
          ...
          $('.overBox1').css('transform-origin', Xper + '% ' + Yper + '%');
          $('.overBox1').css("background-position", thisPosItemX + 'px ' + thisPosItemY + 'px');
      });
      $(document).on(upEvent, function() {
          $(document).off(moveEvent);
          $(document).off(upEvent);
