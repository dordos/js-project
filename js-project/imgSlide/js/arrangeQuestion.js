(() => {
  ...
  class DRAWLINE extends QUIZ {
    ...
    get isCorrect() {
      const userValue = this.drops.map((drop) => !drop.DOM.hasAttribute('data-no-answer') && drop.droppedObj.map((obj) => obj.value)).filter((value) => value);
      userValue.forEach((value) => value.sort());
      this.answerArray.forEach((value) => value.sort());
      console.log(userValue.join(', '), this.answerArray.join(', '))
