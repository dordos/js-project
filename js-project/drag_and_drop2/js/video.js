$(function(){
    ...
    function autoEnd(){
        ...
        $( ".timeCount" ).children().each( function( i ) {
            switch( i ) {
                /*총재생 분 */
                case 6:
    ...
    video.addEventListener('ended', function(){
        ...
        if(ios)
        $('#video1').get(0).webkitExitFullscreen();
    }); // 비디오 끝났을 때
    ...
    onPlaying = function(sTime, eTime, pTime) {
        ...
        if (pTime > 0) {
            $(".timebar").stop( true ).animate({
                width: 100 + "%"
            }, (pTime) * 1000, "linear", function() {
            });
        }
    ...
    $(".timeControl").on(downEvent, function(e) {
        ...
        var parentLeft = ($('.control .bar').offset().left - $('.control').offset().left) + $('.control').offset().left;
