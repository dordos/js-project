...
(() => {
  ...
  function getEventPosition(event) {
    ...
    return { x: eventTarget.clientX, y: eventTarget.clientY }
  }

  function getElementFromPoint(_x, _y) {
    return document.elementFromPoint(getEventPosition(event).x, getEventPosition(event).y);
  }
  ...
  class DrawLine {
    constructor({container, draggableDropElement = true, path = {}}) {
      ...
      watchProperty(() => {
        // this.userValue.value = `0,4 1,3 2,5`;
    ...
    startDrag(__obj, DRAG) {
      ...
      if (__obj.drawedLine) this.removeDrawedLine(__obj);
